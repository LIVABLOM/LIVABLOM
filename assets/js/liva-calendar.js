// ========================================================
// 🌸 LIVA Calendar JS - Version finale (jour de départ libre)
// ========================================================
...
(async function () {
  ...
  function getTarif(dateStr, nbPersonnes=2) {
    const base=79; const supplement=nbPersonnes>2?(nbPersonnes-2)*15:0;
  ...
  function sumPrice(startStr,endStr,nbPersons,testPayment) {
    let total=0; let cur=new Date(startStr); const end=new Date(endStr);
    while(cur<end){ total+=getTarif(cur.toISOString().split("T")[0],nbPersons); cur.setDate(cur.getDate()+1);}
  ...
  document.addEventListener("DOMContentLoaded", async ()=>{
    ...
    let reservedRanges=[]; let selectedStart=null; let selectedEnd=null;
    ...
    function validateForm(){ const name=inputName.value.trim(); const email=inputEmail.value.trim(); const phone=inputPhone.value.trim(); const nbP=parseInt(inputPersons.value); btnConfirm.disabled=!(name&&email&&phone&&nbP>=1&&nbP<=5); }

    [inputName,inputEmail,inputPhone,inputPersons].forEach(el=>el.addEventListener("input",()=>{validateForm();updatePriceDisplay();}));

    const cal=new FullCalendar.Calendar(el,{
      initialView:"dayGridMonth", selectable:true, firstDay:1, locale:"fr", height:"auto",

      dateClick(info){
        const s=new Date(info.dateStr); const e=new Date(s); e.setDate(e.getDate()+1);
        if(!cal.opt("selectAllow")({start:s,end:e})) return; cal.select({start:s,end:e,allDay:true});
      },

      selectAllow(sel){
        const today=new Date(); today.setHours(0,0,0,0); if(sel.start<today) return false;
      ...
      events: async (fetchInfo,success,failure)=>{
        try{
          ...
          reservedRanges=data.map(e=>{
            ...
            end.setDate(end.getDate()-1); // départ libre
            return {start,end};
          });
          success(reservedRanges.map(r=>({title:"Réservé",start:r.start,end:new Date(r.end.getTime()+24*60*60*1000),display:"background",backgroundColor:"#ffcccc",borderColor:"#ff0000",allDay:true})));
      ...
      dayCellDidMount(info){
        ...
        info.el.addEventListener("pointerup",ev=>{
          ...
          const s=new Date(info.el.getAttribute("data-date")); const e=new Date(s); e.setDate(e.getDate()+1);
          if(!cal.opt("selectAllow")({start:s,end:e})) return; cal.select({start:s,end:e,allDay:true});
        },{passive:true});
    ...
    btnConfirm.addEventListener("click",async ()=>{
      const name=inputName.value.trim(); const email=inputEmail.value.trim(); const phone=inputPhone.value.trim(); const nbP=parseInt(inputPersons.value);
      const montant=sumPrice(selectedStart,selectedEnd,nbP,testPayment);
      if(!confirm(`Réserver LIVA du ${selectedStart} au ${selectedEnd} pour ${montant} € ?`)) return;

      const res=await fetch(`${stripeBackend}/api/checkout`,{
        method:"POST",
        headers:{"Content-Type":"application/json"},
        body:JSON.stringify({logement:"LIVA",startDate:selectedStart,endDate:selectedEnd,amount:montant,personnes:nbP,name,email,phone})
      });
      const data=await res.json();
