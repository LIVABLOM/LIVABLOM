// ========================================================
// 🌸 BLŌM Calendar JS - Version Pro (jour départ cliquable)
// ========================================================

(async function () {

  // CSS (sombre, mobile-safe)
  const css = `
  ...
  styleNode.type="text/css"; styleNode.appendChild(document.createTextNode(css));
  ...
  function sumPrice(startStr,endStr,nbPersons,testPayment){
    let total=0, cur=new Date(startStr), end=new Date(endStr);
    while(cur<end){ total+=getTarif(cur.toISOString().split("T")[0],nbPersons,testPayment); cur.setDate(cur.getDate()+1);}
    return total;
  }
  ...
  document.addEventListener("DOMContentLoaded", async ()=>{
    ...
    const config = await getConfig(), testPayment = config.testPayment;

    const modal = document.getElementById("reservationModal"),
          modalDates = document.getElementById("modal-dates"),
          inputName=document.getElementById("res-name"),
          inputEmail=document.getElementById("res-email"),
          inputPhone=document.getElementById("res-phone"),
          inputPersons=document.getElementById("res-persons"),
          priceDisplay=document.getElementById("modal-price"),
          btnCancel=document.getElementById("res-cancel"),
          btnConfirm=document.getElementById("res-confirm");

    let reservedRanges=[], selectedStart=null, selectedEnd=null;

    function updatePriceDisplay(){if(!selectedStart||!selectedEnd)return; const nbPers=parseInt(inputPersons.value)||2; priceDisplay.textContent=`Montant total : ${sumPrice(selectedStart,selectedEnd,nbPers,testPayment)} €`;}
    function validateForm(){const name=inputName.value.trim(),email=inputEmail.value.trim(),phone=inputPhone.value.trim(),nbP=parseInt(inputPersons.value); btnConfirm.disabled=!(name&&email&&phone&&nbP>=1&&nbP<=2);}
    ...
    const cal=new FullCalendar.Calendar(el,{
      ...
      selectAllow(sel){
        ...
        return !reservedRanges.some(r=>sel.start<r.end && sel.end>r.start); // <-- last day now cliquable
      },

      events: async (fetchInfo, success, failure)=>{
        try{
          const res=await fetch(`${calendarBackend}/api/reservations/BLOM`), data=await res.json();
      ...
      dayCellDidMount(info){
        const isReserved=reservedRanges.some(r=>info.date>=r.start && info.date<r.end); // < r.end -> last day cliquable
        if(isReserved){ info.el.setAttribute("data-reserved","true"); return; }

        info.el.addEventListener("pointerup", ev=>{
          if(ev.pointerType==="touch"){ const s=new Date(info.el.getAttribute("data-date")), e=new Date(s); e.setDate(e.getDate()+1); if(!cal.opt("selectAllow")({start:s,end:e})) return; cal.select({start:s,end:e,allDay:true}); }
        },{passive:true});
      ...
      select(info){
        selectedStart=info.startStr.split("T")[0]; selectedEnd=info.endStr.split("T")[0];
        ...
        validateForm(); updatePriceDisplay(); modal.style.display="flex";
      }
    ...
    btnConfirm.addEventListener("click",async ()=>{
      ...
      if(!name||!email||!phone||isNaN(nbP)||nbP<1||nbP>2){alert("Veuillez remplir tous les champs correctement (max 2 personnes)."); return;}
      ...
      try{
        const res=await fetch(`${stripeBackend}/api/checkout`,{method:"POST", headers:{"Content-Type":"application/json"}, body:JSON.stringify({logement:"BLŌM",startDate:selectedStart,endDate:selectedEnd,amount:total,personnes:nbP,name,email,phone})});
